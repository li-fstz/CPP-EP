...
namespace CPP_EP.Lab {
    abstract class Lab {
        ...
        public abstract void Draw (LayoutAnchorablePane layoutAnchorablePane);
    ...
    public class Symbol {
        ...
        public Symbol (string s) {
            Match m = Lab.AddressToSymbol.Match (s);
            if (m.Success) {
                Address = m.Groups[1].Value;
                Name = m.Groups[2].Value;
                Lab.SymbolHash[Address] = this;
            } else {
    ...
    public class Select {
        ...
        public Select (string s) {
            try {
                ...
                foreach (Match m in Lab.Text.Matches (s)) {
                    Symbols.Add (new Symbol (m.Groups[1].Value));
                }
                Address = Symbols[0].Address;
                Lab.SelectHash[Address] = this;
            } catch (Exception e) {
    ...
    public class Rule {
        ...
        public Rule (string s) {
            try {
                ...
                Match m = Lab.AddressToSymbolInQuot.Match (selectStrings[0]);
                Address = m.Groups[1].Value;
                Name = m.Groups[2].Value;
